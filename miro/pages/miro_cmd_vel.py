...
rospy.init_node('streamlit_cmd_vel_control',anonymous=True, disable_signals=True)
...
def control_with_buttons():
    ...
    if forward_button:
        linear_x = 0.5
        angular_z = 0.0
        st.success(f"Moving forward with for {duration} seconds")
    elif backward_button:
        linear_x = -0.5
        angular_z = 0.0
        st.success(f"Moving backward with for {duration} seconds")
    elif left_button:
        linear_x = 0.0
        angular_z = 0.5
        st.success(f"Turning left with for {duration} seconds")
    elif right_button:
        linear_x = 0.0
        angular_z = -0.5
        st.success(f"Turning right with for {duration} seconds")
